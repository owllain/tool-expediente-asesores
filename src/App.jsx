...
import DirectorioLocal from './components/DirectorioLocal'
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-mesh flex text-foreground font-sans selection:bg-primary/30 relative">
      ...
      {/* Sidebar Moderno */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-card border-r border-border flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:h-screen
          ...
          <nav className="space-y-6">
            <div className="space-y-1">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left ${activeView === 'dashboard' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}
              >
                <LayoutDashboard className="h-5 w-5 shrink-0" />
                <span className="font-medium text-sm leading-tight flex-1 whitespace-nowrap">Panel de Control</span>
              </button>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left ${activeView === 'directorio' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}
              >
                <FolderOpen className="h-5 w-5 shrink-0" />
                <span className="font-medium text-sm leading-tight flex-1 whitespace-nowrap">Mis Expedientes</span>
              </button>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left ${activeView === 'registro' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}
              >
                <UserPlus className="h-5 w-5 shrink-0" />
                <span className="font-medium text-sm leading-tight flex-1 whitespace-nowrap">Registro de Expediente</span>
              </button>

              <button
                disabled={!evaluaciones.datosAsesor?.idExpediente}
                onClick={() => {
                  setActiveView('evaluaciones')
                  setIsSidebarOpen(false)
                }}
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left ${
                  !evaluaciones.datosAsesor?.idExpediente 
              ...
              >
                ...
                <span className="font-medium text-sm leading-tight flex-1 whitespace-nowrap">Nuevos Ingresos</span>
              </button>

              <button
                disabled={!evaluaciones.datosAsesor?.idExpediente}
                onClick={() => {
                  setActiveView('partitura')
                  setIsSidebarOpen(false)
                }}
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left ${
                  !evaluaciones.datosAsesor?.idExpediente 
                    ? 'opacity-50 cursor-not-allowed grayscale' 
                    : activeView === 'partitura' 
                      ? 'bg-primary text-white shadow-lg shadow-primary/20' 
                      : 'text-muted-foreground hover:text-white hover:bg-secondary'
                }`}
              >
                <Target className="h-5 w-5 shrink-0" />
                <span className="font-medium text-sm leading-tight flex-1 whitespace-nowrap">Partitura</span>
              </button>
            </div>

            <div className="pt-6 border-t border-border/50">
              <div className="flex items-center gap-2 px-3 mb-3">
                <Settings className="h-3 w-3 text-primary shrink-0" />
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] whitespace-nowrap">Sistema</p>
              </div>
              <button
                onClick={exportData}
                className="w-full flex items-center gap-3 px-3 py-2.5 text-muted-foreground hover:text-white transition-all hover:bg-white/5 rounded-xl group text-left"
              >
                <div className="p-1.5 rounded-lg bg-white/5 group-hover:bg-primary/20 transition-colors shrink-0">
                  <Download className="h-4 w-4" />
                </div>
                <span className="text-sm font-medium leading-tight flex-1 whitespace-nowrap">Descargar Expediente</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-sm font-medium leading-tight flex-1 whitespace-nowrap">Cargar Expediente</span>
              </button>
              <button
                onClick={() => { setActiveView('supervisor'); setIsSidebarOpen(false) }}
              ...
              >
                ...
                <span className="text-sm font-medium leading-tight flex-1 whitespace-nowrap">Modo Supervisor</span>
              </button>
          ...
          <AnimatePresence mode="wait">
                      ...
                      <Tabs value={semanaActual} onValueChange={setSemanaActual}>
                        ...
                        {mesesConfig[mes].semanas.map((semana) => (
                          <TabsContent key={semana.id} value={semana.id}>
                            <NuevosIngresos mes={mes} semana={semana.id} titulo={semana.titulo} descripcion={semana.descripcion} />
                          </TabsContent>
                        ))}
                      </Tabs>
                    </TabsContent>
            ...
            {activeView === 'partitura' && (
              ...
              >
                <Tabs value={semanaActual} onValueChange={setSemanaActual}>
                  <TabsList className="grid grid-cols-4 mb-6 bg-card/30 border border-white/5 p-1 rounded-xl no-print">
                    <TabsTrigger value="semana1" className="text-[10px] font-bold uppercase tracking-tighter border-none shadow-none">Semana 1</TabsTrigger>
                    <TabsTrigger value="semana2" className="text-[10px] font-bold uppercase tracking-tighter border-none shadow-none">Semana 2</TabsTrigger>
                    <TabsTrigger value="semana3" className="text-[10px] font-bold uppercase tracking-tighter border-none shadow-none">Semana 3</TabsTrigger>
                    <TabsTrigger value="semana4" className="text-[10px] font-bold uppercase tracking-tighter border-none shadow-none">Semana 4</TabsTrigger>
                  </TabsList>
                  {['semana1', 'semana2', 'semana3', 'semana4'].map((semana) => (
                    <TabsContent key={semana} value={semana}>
                      <Partitura semana={semana} titulo={`Registro ${semana.replace('semana', 'Semana ')}`} descripcion="Seguimiento de mejora y coaching" />
                    </TabsContent>
                  ))}
                </Tabs>
              </motion.div>
            ...
            {activeView === 'directorio' && (
              <motion.div
                ...
                transition={{ duration: 0.1, ease: "easeInOut" }}
              >
                <DirectorioLocal onSelect={() => setActiveView('dashboard')} />
              </motion.div>
            )}
