...
const Dashboard = () => {
    ...
    const isPartitura = datosAsesor?.tipoProceso === 'partitura'
    const isIngresos = datosAsesor?.tipoProceso === 'ingresos'
    
    let tipoGuiaDisplay = 'Seleccione Tipo de Proceso en Registro'
    if (isPartitura) tipoGuiaDisplay = 'Guía de Partitura'
    if (isIngresos) tipoGuiaDisplay = 'Guía de Nuevos Ingresos'

    const guiaOperacionBase = isPartitura ? [
        { step: '01', text: 'Abra un Registro de Expediente (Tipo: Partitura).' },
        { step: '02', text: 'Documente 4 semanas continuas de evolución.' },
        { step: '03', text: 'Capture hallazgos de calidad, compromisos y retroalimentación.' },
        { step: '04', text: 'Obtenga la firma (o conformidad) del asesor por cada semana.' },
        { step: '05', text: 'Genere un PDF local y utilícelo como respaldo.' },
        { step: '06', text: 'Descargue el Expediente JSON para preservar la integridad.' },
    ] : [
        { step: '01', text: 'Abra un Registro de Expediente (Tipo: Nuevos Ingresos).' },
        { step: '02', text: 'Evalúe integralmente al asesor durante el periodo de 90 días.' },
        { step: '03', text: 'Firme de conformidad al concluir cada una de las 12 semanas.' },
        { step: '04', text: 'Guarde el PDF semanal para la bitácora de evolución.' },
        { step: '05', text: 'Descargue y consolide el Expediente de Datos (Archivo JSON).' },
        { step: '06', text: 'Transmita la resolución final a Jefatura u Operaciones.' },
    ]
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="bg-card/50 border border-white/5 rounded-3xl p-8 backdrop-blur-sm">
                        ...
                        <div className="h-48 flex items-end justify-between gap-3 px-2">
                            {completionByWeek.map((item, idx) => {
                                const progress = typeof item === 'object' ? item.progress || 0 : item || 0
                                const isPartitura = typeof item === 'object' && item.type === 'partitura'
            ...
            <div className="bg-card/30 border border-white/5 rounded-3xl p-8 backdrop-blur-sm">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter">{tipoGuiaDisplay}</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-6">
                    {guiaOperacionBase.map((item, idx) => (
